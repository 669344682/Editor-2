...
function functions.check.FreeCam ()
	return isFreecamEnabled()
end

function functions.check.Mix()
	return isFreecamEnabled() or isThereSelected()
end

function functions.check.Selected()
	return (isThereSelected() and isFreecamEnabled())
end


function functions.check.FreeMove()
	return (isThereSelected() and freeMove)
end

function functions.check.One()
...
function functions.drawCentMenuInfo(image,title,description,x,y,width)
...
function functions.draw.centerMenu()
...
function functions.saveR(format,name)
	callS('save',format,name)
end
...
functions['SaveMenu'] = function (input,exta)
	...
	elseif input == 'Save(OW)' then
		...
		functions.saveR(saveMenu.SType or '.map',exta)
...
function functions.draw.SaveMenu()
	if saveMenu.Open then
		...
		local name = dxDrawEditBox(651*s, 436*s, 229*s, 26*s,1*s,'default',tocolor(0,0,0,197),'Save','',false,tocolor(166, 166, 166, 197),dc)
		...
		local saveE = functions.isCursorOnElement(652*s, 467*s, 105*s, 25*s,'SaveMenu','Save(OW)',name)
		...
		local saveE1 = functions.isCursorOnElement(775*s, 467*s, 105*s, 25*s,'SaveMenu','Save(N)',name)
		local s1Color = (saveE1 and 120 or 210)
		
		dxDrawRectangle(775*s, 467*s, 105*s, 25*s, tocolor(23, 23, 23, s1Color), true)
		dxDrawText("Save as new file", 775*s, 466*s, 880*s, 492*s, tocolor(254, 254, 254, 140), 1.00*s, "default-bold", "center", "center",false,false,true)
	end
end
...
guiTypes['Check box'] = function (wStart,hStart,name,iTable,side,subtract,indexing) 
	...
	global.Checked[name] = global.Checked[name]
