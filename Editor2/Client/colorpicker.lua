...
functions.ColorPicker = function(what,perEntity)
	if perEntity then
		colorpicker[colorpicker.Open][what] = not colorpicker[colorpicker.Open][what]
	else
...
function editBoxC(input,key)
	local table = colorpicker[colorpicker.Open].colorbar
...
colorpicker.cfunction = function ()
	local Cname = colorpicker.Open
	
	colorpicker.start = (xSize-(590*s))--984

	local start = colorpicker.start

	local arrow = functions.isCursorOnElement(start+(256*s), 39*s, 12*s, 12*s,'ColorPicker','Arrow',true)
	...
	dxDrawText(Cname, start+(10*s), 33*s, start+(304*s), 57*s, tocolor(255, 255, 255, 255), 1.00*s, "default-bold", "left", "center", false, true, false, false, false)
	...
	if colorpicker[Cname]['Arrow'] then

		dxDrawRectangle(start, 58*s, 304*s, 202*s, tocolor(0, 0, 0, 141), false)

		local r,g,b,hA = unpack(colorpicker[Cname].color)
		local ra,ga,ba,wB,hB,hex = unpack(colorpicker[Cname].colorbar)
...
function trigger ( _,_,x, y,_,_,_,ignore,click )
	...
	if colorpicker[colorpicker.Open] then
		if colorpicker[colorpicker.Open]['Arrow'] then
			if getKeyState('mouse1') then
				...
				if (not ignore) and (not SelectedB) then
					if functions.isCursorOnElement(colorpicker.start+(201*s), 0, 25*s, 300*s) and ((not SelectedB) or ignore) then
						...
						if r then
							colorpicker[colorpicker.Open].color = {r,g,b,y}

							local _,_,_,xA,yA = unpack(colorpicker[colorpicker.Open].colorbar)
							if tonumber(xA) then
				...
				if ((ignore) or functions.isCursorOnElement(colorpicker.start+(5*s), 63*s, 193*s, 193*s)) and ((not SelectedA) or ignore) then
					...
					if r then
						...
						local rA,gA,bA = unpack(colorpicker[colorpicker.Open].color)
						local ra,ga,ba = (rA/255)*ab ,(gA/255)*ab ,(bA/255)*ab
						local hex = rgb2hex(math.floor(ra),math.floor(ga),math.floor(ba))
						colorpicker[colorpicker.Open].colorbar = {ra+r,ga+g,ba+b,x,y,hex}
					end
				end
				triggerEvent ('Color', resourceRoot,colorpicker.Open,colorpicker[colorpicker.Open].colorbar)
			end
		end
	end
end
...
function addLabelOnClick ( button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement )
	if state then
		trigger(nil,nil,absoluteX,absoluteY,nil,nil,nil,nil,true)
